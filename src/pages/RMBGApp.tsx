...
const RMBGAppWorker: React.FC<RMBGAppProps> = ({ onBack }) => {
  ...
  const [backgroundType, setBackgroundType] =
    useState<string>("checkerboard-dark"); // 背景类型
  ...
  const getBackgroundStyle = () => {
    const bg =
      backgroundStyles[backgroundType as keyof typeof backgroundStyles];
              ...
              <div className="background-picker-options">
                {Object.entries(backgroundStyles).map(([key, config]) => (
                  <button
                    ...
                    title={config.name}
                  >
                    <div className="background-preview" style={config.style} />
                  </button>
                ))}
              </div>
            </div>
